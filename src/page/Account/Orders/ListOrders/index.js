...
export const ListOrders = (props) => {
    ...
    return (
        <ul className={listOrder.length === 0 ? "bgW" : ""} >
            {listOrder.length === 0 && <img src="https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcSqi0CV31qQN4mugKP896IvQbXar1O34Nd0fhneN3YPih37t2b9wnCKYB_7FKTgjI0fAZc&usqp=CAU" className="order-empty" />}
            {listOrder?.map(order => {
                return (
                    <li key={order._id}>
                        ...
                        {order?.productDetail.map(product => {
                            ...
                            return (
                                <div className="order-detail" key={product._id}>
                                    <img src={product?.productID.urls[0].url} />
